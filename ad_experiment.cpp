...
int main(int argc, char const *argv[])
{
    ...
    ifstream inFile("/data/german/data.csv");
    string lineStr;
    ...
    ifstream inFile2("/data/german/ID_ALL.csv");
    string lineStr2;
    ...
    //baseline
    ofstream outFile;
    outFile.open("/german_ad1.csv", ios::out); 
    ...
    for (int t = 0; t < 20; t++)
    {
        ...
        RBP test_RBP(B,sigma);
        BPAS test_BPAS(B,C_BPAS,sigma);
        Projectron test_proj(B,sigma);
        BOGD test_BOGD(eta,B,lambda,gamma,sigma);


        MatrixXf dataX = MatrixXf::Zero(kb*kr,dim_x-1);
        ...
        for (int i = 0; i < kb; i++)
        {
            ...
            for (int j = 0; j < kr; j++)
            {
                for (int p = 1; p < dim_x-1; p++)
                {
                    dataX(kr*i+j,p-1) = strArray[sample_index][p];
                }
                if (i%2 == 1)
                {
                    dataY.push_back(-1*strArray[sample_index][0]);
                }
                else
                {
