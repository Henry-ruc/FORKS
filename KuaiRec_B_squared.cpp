...
int main(int argc, char const *argv[])
{
    ...
    ifstream inFile("/home/dongxie_wen/Kuairec/KuaiRuc_version_2.csv");
    string lineStr;
    ...
    ofstream outFile;
    outFile.open("/home/dongxie_wen/exp/kuai_B_squared.csv", ios::out); 
    ...
    for (int B_loop = 0; B_loop < n_B; B_loop++)
    {
        ...
        std::vector<float> time_FOGD;
        std::vector<float> wrong_rate_FOGD;
        for (int t = 0; t < 1; t++)
        {
            //SkeGD setting
            int B = B_set[B_loop];
            ...
            system_clock::time_point beg_t_FOGD = system_clock::now();
            test_FOGD.FOGD_learning(dataX,Y);
            system_clock::time_point end_t_FOGD = system_clock::now();
            duration<double> diff_FOGD = end_t_FOGD - beg_t_FOGD;
            time_FOGD.push_back(diff_FOGD.count());
            wrong_rate_FOGD.push_back(float(test_FOGD.wrong)/(N));
        }
        ...
        //FOGD
        double sum = std::accumulate(std::begin(time_FOGD), std::end(time_FOGD), 0.0);  
        double mean_time_FOGD =  sum / time_FOGD.size(); 
        sum = std::accumulate(std::begin(wrong_rate_FOGD), std::end(wrong_rate_FOGD), 0.0);  
        double mean_FOGD =  sum / wrong_rate_FOGD.size(); 
        outFile << mean_time_FOGD << ',' << mean_FOGD << endl;
    }
    outFile.close();
